...
public class MySQLVerticle extends AbstractVerticle {
	...
	protected void getGroup(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.groups WHERE idGroup = ?;", Tuple.of(idGroup), res -> {
			if (res.succeeded()) {
				// Get the result set
	...
	protected void deleteGroup(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM dad.groups WHERE idGroup = ?;", Tuple.of(idGroup), res -> {
			if (res.succeeded()) {
	...
	protected void getDevice(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.devices WHERE idDevice = ?;", Tuple.of(idDevice), res -> {
			if (res.succeeded()) {
				// Get the result set
				RowSet<Row> resultSet = res.result();
				Device device = null;
	...
	protected void deleteDevice(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM dad.devices WHERE idDevice = ?;", Tuple.of(idDevice), res -> {
			if (res.succeeded()) {
	...
	protected void getSensorsFromDeviceId(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.sensors WHERE idDevice = ?;", Tuple.of(idDevice), res -> {
			if (res.succeeded()) {
				// Get the result set
				RowSet<Row> resultSet = res.result();
	...
	protected void getActuatorsFromDeviceId(int idDevice, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.actuators WHERE idDevice = ?;", Tuple.of(idDevice),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void getActuatorsFromDeviceIdAndActuatorType(DatabaseMessageIdAndActuatorType queryParams,
			DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.actuators WHERE idDevice = ? AND actuatorType = ?;",
	...
	protected void addDeviceToGroup(Device device, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("UPDATE dad.devices SET idGroup = ? WHERE idDevice = ?;",
	...
	protected void getDevicesFromGroupId(int idGroup, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.devices WHERE idGroup = ?;", Tuple.of(idGroup), res -> {
			if (res.succeeded()) {
				// Get the result set
				RowSet<Row> resultSet = res.result();
	...
	protected void getSensor(int idSensor, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.sensors WHERE idSensor = ?;", Tuple.of(idSensor), res -> {
			if (res.succeeded()) {
				// Get the result set
				RowSet<Row> resultSet = res.result();
				Sensor sensor = null;
	...
	protected void createSensorValue(SensorValue sensorValue, DatabaseMessage databaseMessage,
		...
		mySqlClient.preparedQuery(
				"INSERT INTO dad.sensorValues (value, idSensor, timestamp, removed) VALUES (?,?,?,?);",
	...
	protected void getLastSensorValueFromSensorId(int idSensor, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM dad.sensorValues WHERE idSensor = ? ORDER BY `timestamp` DESC LIMIT 1;",
	...
	protected void createActuator(Actuator actuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO dad.actuators (name, idDevice, actuatorType, removed) VALUES (?,?,?,?);",
	...
	protected void getActuator(int idActuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("SELECT * FROM dad.actuators WHERE idActuator = ?;", Tuple.of(idActuator),
				res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
	...
	protected void editActuator(Actuator actuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"UPDATE dad.actuators g SET name = COALESCE(?, g.name), idDevice = COALESCE(?, g.idDevice), actuatorType = COALESCE(?, g.actuatorType), removed = COALESCE(?, g.removed) WHERE idActuator = ?;",
	...
	protected void deleteActuator(int idActuator, DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM dad.actuators WHERE idActuator = ?;", Tuple.of(idActuator),
				res -> {
					if (res.succeeded()) {
	...
	protected void createActuatorStatus(ActuatorStatus actuatorState, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery(
				"INSERT INTO dad.actuatorStates (status, statusBinary, idActuator, timestamp, removed) VALUES (?,?,?,?,?);",
	...
	protected void deleteActuatorStatus(int idActuatorStatus, DatabaseMessage databaseMessage,
			Message<Object> message) {
		mySqlClient.preparedQuery("DELETE FROM dad.ActuatorStates WHERE idActuatorState = ?;",
	...
	protected void getLatestActuatorStatesFromActuatorId(DatabaseMessageLatestValues queryParam,
			DatabaseMessage databaseMessage, Message<Object> message) {
		mySqlClient.preparedQuery(
				"SELECT * FROM dad.actuatorStates WHERE idActuator = ? ORDER BY `timestamp` DESC LIMIT ?;",
				Tuple.of(queryParam.getId(), queryParam.getLimit()), res -> {
					if (res.succeeded()) {
						// Get the result set
						RowSet<Row> resultSet = res.result();
